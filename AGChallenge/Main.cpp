...
void main(int iArgCount, char **ppcArgValues)
{
	...
	// vRunIsingSpinGlassExperiment(784, 199, i_mask_seed);
	// vRunIsingSpinGlassExperiment(81, 0, i_mask_seed);
	// vRunIsingSpinGlassExperiment(81, 0, iSEED_NO_MASK);

	// vRunLeadingOnesExperiment(1000, i_mask_seed);
	// vRunLeadingOnesExperiment(1000, iSEED_NO_MASK);

	// vRunMaxSatExperiment(1000, 0, 4.27f, i_mask_seed);
	//vRunMaxSatExperiment(1000, 0, 4.27f, iSEED_NO_MASK);

	// vRunNearestNeighborNKExperiment(500, 0, 4, i_mask_seed);
	// vRunNearestNeighborNKExperiment(200, 0, 4, iSEED_NO_MASK);

	// vRunOneMaxExperiment(1000, i_mask_seed);
	// vRunOneMaxExperiment(1000, iSEED_NO_MASK);

	vRunRastriginExperiment(1000, 10, i_mask_seed);
	// vRunRastriginExperiment(1000, 10, iSEED_NO_MASK);
}//void main(int iArgCount, char **ppcArgValues)